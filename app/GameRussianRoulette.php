	...
	function shoot(){
		...
		if(empty($shot)){
			...
			if($shot == 6 && $last != -1){
				...
				$this->chat->settings('russian_roulette', 0); // DELETE
				$text = ":dizzy_face: :boom::gun:";
			}elseif($last != -1){
				// $this->analytics->event('Telegram', 'Games', 'Roulette Shot');
				$this->chat->settings('russian_roulette', ($shot + 1));
				$faces = ["happy", "stuck_out_tongue", "smiley"];
				$r = mt_rand(0, count($faces) - 1);
				$text = ":" .$faces[$r] .": :dash::gun:";
			}
			$this->telegram->send
				->notification(FALSE)
				->reply_to(TRUE)
			...
			if($shot == 6 && $last != -1 && $this->telegram->is_chat_group()){
				...
				// Avisar al admin?
				$this->chat->settings('russian_roulette_last', 0);
			}
		}
	}
}
