...
class Main extends TelegramApp\Module {
	...
	public function language($set = NULL){
		...
		$this->telegram->send
			->inline_keyboard()
				->row()
					->button(json_encode($flags['es']), "language es", "TEXT")
					->button(json_encode($flags['en']), "language en", "TEXT")
				->end_row()
				->row()
					->button(json_encode($flags['fr']), "language fr", "TEXT")
					->button(json_encode($flags['it']), "language it", "TEXT")
					->button(json_encode($flags['de']), "language de", "TEXT")
				->end_row()
			->show()
			->text($str)
		->send();
